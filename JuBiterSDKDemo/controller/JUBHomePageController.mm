...
- (void) buttonConfiguration {
    ...
    JUB_NS_ENUM_COMMODE comMode  = JUB_NS_ENUM_COMMODE::NS_COMMODE_NS_ITEM;
    JUB_NS_ENUM_DEVICE deviceClass = JUB_NS_ENUM_DEVICE::NS_DEVICE_NS_ITEM;
    DeviceTypeInfo* deviceTypeInfo = [g_sdk JUB_GetDeviceType:[sharedData currDeviceID]];
    rv = [g_sdk lastError];
    if (JUBR_OK != rv) {
        [self addMsgData:[NSString stringWithFormat:@"[JUB_GetDeviceType() return %@ (0x%lx).]", [JUBErrorCode GetErrMsg:rv], rv]];
    }
    else {
        comMode = deviceTypeInfo.commode;
        deviceClass = deviceTypeInfo.category;
        
        [self addMsgData:[NSString stringWithFormat:@"[JUB_GetDeviceType(%ld:%ld) OK.]", comMode, deviceClass]];
        ...
        BOOL disEnable = NO;
        if (JUB_NS_ENUM_DEVICE::NS_BIO != deviceClass) {
            disEnable = YES;
        }
...
- (void) rightNAVButtonClicked {
    ...
    JUBListAlert *listAlert = [JUBListAlert showCallBack:^(NSString *_Nonnull selectedItem) {
        ...
        [g_sdk JUB_isDeviceConnect:currDeviceID];
        rv = [g_sdk lastError];
        if (JUBR_OK != rv) {
...
//        });
        ...
        {
            currDeviceID = [optDeviceID intValue];
            [g_sdk JUB_disconnectDevice:currDeviceID];
            rv = [g_sdk lastError];
            if (JUBR_OK != rv) {
