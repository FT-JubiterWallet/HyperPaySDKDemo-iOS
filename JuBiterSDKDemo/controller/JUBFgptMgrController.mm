...
- (void)viewDidLoad {
    ...
    JUB_UINT16 deviceID = [[[JUBSharedData sharedInstance] currDeviceID] intValue];
...
#pragma mark - 业务
- (JUB_RV)verify_fgpt_test:(JUB_UINT16)deviceID {
...
- (void)navRightButtonCallBack {
    ...
    if (JUBR_OK != [self verify_fgpt_test:[[[JUBSharedData sharedInstance] currDeviceID] intValue]]) {
        return;
    }
}
...
//    }];
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            fingerEntryAlert.fingerNumber = enrollInfo.nextIndex;
        });
        
        for (JUB_BYTE i=enrollInfo.nextIndex; i<enrollInfo.times; ++i) {
            times = enrollInfo.times;
            
            NSLog(@"for-enrollInfo.nextIndex = %ld.", enrollInfo.nextIndex);
